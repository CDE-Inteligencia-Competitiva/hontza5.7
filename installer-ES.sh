...
download(){
      ...
      mv "/home/hontza5.7-master/src" './hontza-source'
...
rootUser(){
    ROOTUSER='root'
    echo
    echo  ".....Cuál es el Username con derechos de admin en la base de datos (mysql)? [${ROOTUSER}]"
    read BUF
    if [ -n "$BUF" ]
    then
      ROOTUSER="$BUF"
...
chown -R www-data:www-data "${WEBROOT}/${WEBFOLDER}/sites"
chown -R www-data:www-data "${WEBROOT}/${WEBFOLDER}/languages"
chown -R www-data:www-data "${WEBROOT}/${WEBFOLDER}/google_sheet"
...
echo -e "Se creara una carpeta temporal para el proceso de instalación [/tmp/hwork]"
...
echo
echo "-Nombre de la base de datos de Hontza: Hontza"

       
echo "Inserte el nuevo password para el usuario hontza de la base de datos hontza:"
