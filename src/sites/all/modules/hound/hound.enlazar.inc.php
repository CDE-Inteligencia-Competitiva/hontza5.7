...
} //window.open(hound_enlazar_url,"_self");
function hound_enlazar_inc_add_create_js(){
    ...
    $js='var hound_enlazar_url="'.$url.'";
        ...
        $("#edit-hound1-create-btn").click(function(){     
             window.open(hound_enlazar_url,"_blank","toolbar=no, location=yes, hidden=yes, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width=800, height=600");
            return false;
...
function hound_enlazar_inc_get_create_url(){
    ...
    $url.='/channels/crearCanal/'.hound_enlazar_inc_get_key().'?subdominio='.$base_url.'&grupo='.$my_grupo->purl;
...
function get_group(){
    $my_grupo=og_get_group_context(); 
    return $my_grupo->purl;
}
function hound_enlazar_inc_get_key(){
    //joseba
    $hound_key=db_result(db_query('SELECT gakoa FROM {key_empresa}'));
    return $hound_key;
}

function get_nombre_hound(){
    if($_GET['channel_name']!=null) {
...
function get_id_hound(){
    if($_GET['channel_id']!=null) {
...
function hound_enlazar_inc_get_edit_url($canal){
    ...
    $url.='/channels/editarCanal/'.$hound_id.'/'.hound_enlazar_inc_get_key().'?subdominio='.$base_url.'&grupo='.$my_grupo->purl.'&edit=1';
    return $url;
}
...
function hound_enlazar_inc_get_delete_url($canal){
    ...
    //$url.='/channels/borrarCanal/'.$hound_id;
    drupal_set_message(t('The asocciated hound channel has been deleted'));
    return $url;
}
