...
function hound_javascript_crear_canal_callback(){
    ...
    $hound_group=arg(6);
    $key_enpresa=arg(7);
    if ($javascript_type=="editar"){
        drupal_goto($base_url.'/'.$hound_group.'/vigilancia/pendientes');
    }
    drupal_goto($base_url.'/'.$hound_group.'/crear/canal-yql?simple=1&is_hound=1&channel_name='.$hound_name.'&channel_id='.$hound_id);
    //$inprimatzeko=file_get_contents('http://46.26.232.77/hound2/Query.php?channels='.$hound_id);
    //$inprimatzeko=file_get_contents('http://192.168.0.159/hound2/Query.php?channels='.$hound_id);
    //hound_javascript_crear_canal($hound_id,$javascript_type,$hound_name,$hound_group,$inprimatzeko,$key_enpresa);
    //sortu berri den canala eskrapeatzeko
    
}
function hound_javascript_crear_canal($hound_id,$javascript_type,$hound_name,$hound_group,$inprimatzeko,$key_enpresa){
    global $base_url;
...
<h2> Tu código del canal es: <b>'.$hound_id.'</b> </h2>'.$javascript.' 
 ...
 <h3> <a href="'.$base_url.'/'.$hound_group.'/crear/canal-yql?simple=1&is_hound=1&channel_name='.$hound_name.'&channel_id='.$hound_id.'"> Exportar canal a hontza</a></h3>

 <h3> <a href="http://hound.hontza.es/hound2/public/channels/borrarCanal/'.$hound_id.'/'.$key_enpresa.'"> Borrar canal de hound</a></h3>
