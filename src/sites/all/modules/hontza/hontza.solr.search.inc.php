...
function hontza_solr_search_get_pdf($node_id_array,$url_return,&$is_return,&$title,$is_solr=0){
    $html=array();
    if(!hontza_solr_search_modificar_taxonomia_access()){
        drupal_access_denied();
        exit();
    ...
    $is_return=0;
    $title=t('Informe Realizado');

    $pdf_title ='Informe realizado';
    $send_filename=time().'.pdf';     
    $boletin_report_array_edit_id=$node_id_array;    
    $text_row='';
    ...
    //$content=boletin_report_get_content_html($is_download,$is_print_exit,'426258','0');
  //  $content=boletin_report_get_content_html($is_download,$is_print_exit,$node_id_array,$boletin_report_array_edit_id);
 
   // $res=db_query("SELECT title from node_revision where nid=426258"); 
    $num=count($node_id_array);
    $html=array();
    $data=date("F j, Y, g:i a");    
    $html[]='<h1>INFORME '.$data.' </h1>';
    for($i=0;$i<$num;$i++){
   
    $my_node=node_load($node_id_array[$i]); 
    //$title=db_fetch_object($res);
    $link='';  
    $url1=crm_exportar_get_node_url($link,$my_node);
    
     $html[]='<td>';  
    $html[]='<h4>'.$my_node->title.'<h4>';
     $html[]='<h5>'.$my_node->body.'<h5>';
...
/*    //$html[]=hontza_solr_funciones_get_selected_node_titles($node_id_array);
    ...
    $pdf -> Output('archivo_pdf.pdf', 'D'); //Generamos el archivo "archivo_*/
   // return implode('',$html);
}
...
function hontza_solr_search_get_bookmark_mode_js(){
    return 'function hontza_solr_funciones_get_bookmark_mode(my_id){
        if(my_id=="id_backup_channel_bookmark_multiple_mode"){
            ...
            return "rechazar";
        }else if(my_id=="id_get_pdf_bookmark_multiple_mode"){
            return "pdf";
