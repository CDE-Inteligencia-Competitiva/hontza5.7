        ...
        if(isset($node->nid) && !empty($node->nid)){
            ...
            if($is_hound_filter_activado){
                ...
                if(hontza_is_hound_canal($node->nid)){
                   ...
                   hontza_canal_de_yql_presave($node); //joseba2019 bestela hounda filtro biurtzen da (desactivatuta alare)
                }
            }
        }    
    }
...
function hontza_yql_field_nombrefuente_canal_form_alter(&$form,&$form_state,$form_id){
    ...
    if($is_show_filtros_rss){
        ...
        if(count($rss_array)>1){
            ...
            //joseba2019nameofsource kentzeko 
...
function hontza_get_rss_array_by_field_nombrefuente_canal($form){
    ...
    if(isset($form['field_nombrefuente_canal']) && isset($form['field_nombrefuente_canal'][0]) && isset($form['field_nombrefuente_canal'][0]['#default_value']) && isset($form['field_nombrefuente_canal'][0]['#default_value']['value'])){
       ...
       // $result=explode('·',$value);
        $value=$form['feeds']['FeedsHTTPFetcher ']['source']['#default_value'];

        //hemen separatu behar dugu url composatua, lehen zegona 
        //funtzionatzen du baina arazoekin. 
        
        
        
        
        $urlsep= explode("&",$value);
        //obtener urls

        $urlsep1=explode("?",$urlsep[0]);
        $urlak=substr($urlsep1[1],4,strlen($urlsep1[1]));
        $result=explode(",",$urlak);

        
        if(count($result)>0){
            foreach($result as $i=>$v){
                $result[$i]=base64_decode(trim($v));
            }
        }
    }
    return $result;
    ...
    if(in_array($node->type,array('canal_de_supercanal'))){
        ...
        if(isset($node->field_nombrefuente_canal[0]['value'])){					
            ...
            if($num>0){
                ...
                $output=$node->field_nombrefuente_canal[0]['value'];
...
//joseba2019 
function hontza_is_filter($node){
    $feed_source=get_feeds_source($node->nid);
    $url=$feed_source->source;
    //print_r($url);
    if (strpos($url,'rssHontza')===false){
        return 0;
    }else{
        return 1;
    }
...
function hontza_get_url_rss($node){
    $feed_source=get_feeds_source($node->nid);
    $url=$feed_source->source;
    //$url=$node->field_nombrefuente_canal[0]['value'];
    $urlsep= explode("&",$url);
...
function hontza_get_and_rss($node,$where){
    $feed_source=get_feeds_source($node->nid);
    $url=$feed_source->source;
    //$url=$node->field_nombrefuente_canal[0]['value'];
    $urlsep= explode("&",$url);
    ...
    for($t=0;$t<count($urlsep);$t++){
            $filter1=$urlsep[$t];
            if($filter1[0]=='a'){
                $tartea=strlen($filter1)-4;
                $and1=substr($filter1,4,$tartea);
                $and1=base64_decode($and1);
                $andak=explode(",",$and1);
                for($o=0;$o<count($andak);$o++){
                    ...
                    if($att[0]=='or1'){
                        if($att[1]=='title'){
                            if(strlen($arg1title)==0){
                                if (strpos($sep[0],'+')==FALSE){
                                $tartea=strlen($sep[0])-8;
                                $hitza=substr($sep[0],3,$tartea);
                                }else{
                                $tartea=strlen($sep[0])-6;
                                $hitza=substr($sep[0],3,$tartea);
                                }
                                $arg1title=$hitza;
                            }else{
                        ...
                        }elseif($att[1]=='description'){
                            if(strlen($arg1description)==0){
                                if (strpos($sep[0],'+')==FALSE){
                                $tartea=strlen($sep[0])-8;
                                $hitza=substr($sep[0],3,$tartea);
                                }else{
        ...
        $andarray=array();
        $andarray['arg1title']=$arg1title;
        $andarray['arg2title']=$arg2title;
        $andarray['arg3title']=$arg3title;
        $andarray['arg1description']=$arg1description;
        $andarray['arg2description']=$arg2description;
        $andarray['arg3description']=$arg3description;
        $andarray['arg1link']=$arg1link;
        $andarray['arg2link']=$arg2link;
        $andarray['arg3link']=$arg3link;
        $value=$andarray[$where];
        return $value;


}
function hontza_get_or_rss($node,$where){
    $feed_source=get_feeds_source($node->nid);
    $url=$feed_source->source;
   // $url=$node->field_nombrefuente_canal[0]['value'];
    $urlsep= explode("&",$url);
    ...
    for($t=0;$t<count($urlsep);$t++){
        $filter1=$urlsep[$t];
        if($filter1[0]=='f'){
        $tartea=strlen($filter1)-7;
        $filter2=substr($filter1,7,$tartea);
        $filter2=base64_decode($filter2);
        $filter3=explode(",", $filter2);
        for ($i=0;$i<count($filter3);$i++){
            $divisoria=explode("_",$filter3[$i]);
            if($divisoria[1]=='title'){
                if(strlen($ortitle)==0){
                    if (strpos($divisoria[0],'+')==FALSE){
                        $tartea=strlen($divisoria[0])-8;
                        $hitza=substr($divisoria[0],3,$tartea);
                    }else{
                        $tartea=strlen($divisoria[0])-6;
                        $hitza=substr($divisoria[0],3,$tartea);
                    }
                    $ortitle=$hitza;
                }
            ...
            if($divisoria[1]=='description'){
                if(strlen($ordescription)==0){
                    if (strpos($divisoria[0],'+')==FALSE){
                        $tartea=strlen($divisoria[0])-8;
                        $hitza=substr($divisoria[0],3,$tartea);
                    }else{
    ...
    $orarray=array();
    $orarray['ortitle']=$ortitle;
    $orarray['ordescription']=$ordescription;
    $orarray['orlink']=$orlink;
    $value=$orarray[$where];
    return $value;

}
function hontza_get_not_rss($node,$where){
   ...
   // $url=$node->field_nombrefuente_canal[0]['value'];
    ...
    for($t=0;$t<count($urlsep);$t++){
            ...
            if($filter1[0]=='n'){
                $tartea=strlen($filter1)-4;
                $not1=substr($filter1,4,$tartea);
                $not1=base64_decode($not1);
                $not2=explode(",",$not1);
                for ($y=0;$y<count($not2);$y++){
                    $divisor=explode("_",$not2[$y]);
                    if($divisor[1]=='title'){
                        if(strlen($nottitle)==0){
                            if (strpos($divisor[0],'+')==FALSE){
                                $tartea=strlen($divisor[0])-8;
                                $hitza=substr($divisor[0],3,$tartea);
                                }else{
                                $tartea=strlen($divisor[0])-6;
                                $hitza=substr($divisor[0],3,$tartea);
                            }
                            $nottitle=$hitza;
                        }else{
                    ...
                    }elseif($divisor[1]=='description'){
                        if(strlen($notdescription)==0){
                            if (strpos($divisor[0],'+')==FALSE){
                                $tartea=strlen($divisor[0])-8;
                                $hitza=substr($divisor[0],3,$tartea);
                                }else{
    ...
    $notarray=array();
    $notarray['nottitle']=$nottitle;
    $notarray['notlink']=$notlink;
    $notarray['notdescription']=$notdescription;
    $value=$notarray[$where];
    return $value;
...
function hontza_modify_url_rss_multiple($data,$rss_string_list,$url_rss,&$form_state){
    ...
    //joseba2019 filtroa hound-ari ez aplikatzeko. 
    if(strpos($url_rss,'/channels/getRSS/')===false){
        $result=my_encode_yql_filter($form,$form_state);
        if($result==1){
            $result=$url_rss;
        }
...
function hontza_set_form_filtros_yql($is_hound,$is_edit,&$form,$form_state=''){
    ...
    if($is_edit){
        ...
        //return help_popup_window(138308,'help',my_get_help_link_object());
    ...
    $nodea=hontza_get_node_by_form($form);
   ...
   // $url=$node->field_nombrefuente_canal[0]['value'];
     ...
     $urlcomposatua= $form['feeds']['FeedsHTTPFetcher']['source']['#default_value'];
    ...
    $separazioa2=$urlsep[2];
    if($separazioa2[0]=='a'){
        $tartea=strlen($separazioa2)-4;
        $and1=substr($separazioa2,4,$tartea);
        $and1=base64_decode($and1);
        $andak=explode(",",$and1);
        for($o=0;$o<count($andak);$o++){
            $sep=explode("_",$andak[$o]);
    ...
    }elseif($separazioa2[0]=='n'){
        $tartea=strlen($separazioa2)-4;
        $not1=substr($separazioa2,4,$tartea);
        $not1=base64_decode($not1);
        $not2=explode(",",$not1);
        for ($y=0;$y<count($not2);$y++){
            $divisor=explode("_",$not2[$y]);
    ...
    }elseif($separazioa2[0]=='r'){
        $reg=1;
    }
    $separazioa3=$urlsep[3];
    if($separazioa3[0]=='n'){
        $tartea=strlen($separazioa3)-4;
        $not1=substr($separazioa3,4,$tartea);
        $not1=base64_decode($not1);
        $not2=explode(",",$not1);
        for ($y=0;$y<count($not2);$y++){
            $divisor=explode("_",$not2[$y]);
...
*/
//collapsed-ak definitzeko.
    if($arg1title!=='' || $arg2title!=='' || $arg3title!=='' || $arg1description!=='' || $arg2description!=='' || $arg3description!=='' || $arg1link!=='' || $arg2link!=='' || $arg3link!==''){
        $andcollapsed=FALSE;
    }else{
        $andcollapsed=TRUE;
    }

    if($ortitle!=='' || $ordescription!=='' || $orlink!==''){
        $orcollapsed=FALSE;
    }else{
        $orcollapsed=TRUE;
    }
    if($nottitle!=='' || $notdescription!=='' || $notlink!==''){
        $notcollapsed=FALSE;
    }else{
        $notcollapsed=TRUE;
    }
...
*/    
    $form['and']=array(
        ...
        The red blocks are combined with AND.'),
         '#attributes' => array(
 ...
 $form['and']['or']=array(
        '#type'=>'fieldset',
        '#title'=>'OR',
        '#description' =>t('Terms in the same box are combined with boolean OR. </BR></BR>Boxes in the same Block are combined with boolean OR'),
        '#attributes' => array(
...
function my_encode_yql_filter(&$form,&$form_state){
    ...
    if($form_state['yql_obj']->filtros['url']){
        $urls=$form_state['yql_obj']->filtros['url'];
        $urla=explode(",",$urls);
   ...
   // if($form_state['yql_obj']->filtros['regular']==TRUE){
      $reg=1;
   // }else{
   //   $reg=0;
   // }

    //sacando argumentos not

    if($form_state['yql_obj']->filtros['nottitle']){
        $sep=explode(",",$form_state['yql_obj']->filtros['nottitle']);
    ...
    if($form_state['yql_obj']->filtros['notdescription']){
        $sep=explode(",",$form_state['yql_obj']->filtros['notdescription']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x], '+')==FALSE){
                if($x==0 && !isset($not)){
    ...
    if($form_state['yql_obj']->filtros['notlink']){
        $sep=explode(",",$form_state['yql_obj']->filtros['notlink']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x], '+')==FALSE){
                if($x==0 && !isset($not)){
    ...
    if($form_state['yql_obj']->filtros['ortitle']){
        $sep=explode(",",$form_state['yql_obj']->filtros['ortitle']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x], '+')==FALSE){
    ...
    if($form_state['yql_obj']->filtros['ordescription']){
        $sep=explode(",",$form_state['yql_obj']->filtros['ordescription']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x], '+')==FALSE){
                if($x==0 && !isset($filtera)){
    ...
    if($form_state['yql_obj']->filtros['orlink']){
        $sep=explode(",",$form_state['yql_obj']->filtros['orlink']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x], '+')==FALSE){
                if($x==0 && !isset($filtera)){
    ...
    if($form_state['yql_obj']->filtros['arg1title']){
        $sep=explode(",",$form_state['yql_obj']->filtros['arg1title']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x],'+')==FALSE){
                if($x==0 && !isset($and)){
    ...
    if($form_state['yql_obj']->filtros['arg1description']){
        $sep=explode(",",$form_state['yql_obj']->filtros['arg1description']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x],'+')==FALSE){
                if($x==0 && !isset($and)){
    ...
    if($form_state['yql_obj']->filtros['arg1link']){
        $sep=explode(",",$form_state['yql_obj']->filtros['arg1link']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x],'+')==FALSE){
                if($x==0 && !isset($and)){
    ...
    if($form_state['yql_obj']->filtros['arg2title']){
        $sep=explode(",",$form_state['yql_obj']->filtros['arg2title']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x],'+')==FALSE){
                if($x==0 && !isset($and)){
    ...
    if($form_state['yql_obj']->filtros['arg2description']){
        $sep=explode(",",$form_state['yql_obj']->filtros['arg2description']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x],'+')==FALSE){
                if($x==0 && !isset($and)){
    ...
    if($form_state['yql_obj']->filtros['arg2link']){
        $sep=explode(",",$form_state['yql_obj']->filtros['arg2link']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x],'+')==FALSE){
                if($x==0 && !isset($and)){
    ...
    if($form_state['yql_obj']->filtros['arg3title']){
        $sep=explode(",",$form_state['yql_obj']->filtros['arg3title']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x],'+')==FALSE){
                if($x==0 && !isset($and)){
    ...
    if($form_state['yql_obj']->filtros['arg3description']){
        $sep=explode(",",$form_state['yql_obj']->filtros['arg3description']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x],'+')==FALSE){
                if($x==0 && !isset($and)){
    ...
    if($form_state['yql_obj']->filtros['arg3link']){
        $sep=explode(",",$form_state['yql_obj']->filtros['arg3link']);
        for($x=0;$x<count($sep);$x++){
            if(strpos($sep[$x],'+')==FALSE){
                if($x==0 && !isset($and)){
    ...
    /*
        if($form_state['yql_obj']->filtros['arg1and']){
            $and1=$form_state['yql_obj']->filtros['arg1and'];
        ...
        if($form_state['yql_obj']->filtros['arg2and']){
            $and2=$form_state['yql_obj']->filtros['arg2and'];
        ...
        if($form_state['yql_obj']->filtros['arg3and']){
            $and3=$form_state['yql_obj']->filtros['arg3and'];
   ...
   if(isset($and)){
          if (isset($filtera)){
              ...
              $urla='http://localhost/hontza/rssHontza/filter.php?url='.$url.'&filter='.$filtera.'&and='.$and.'&not='.$not.'&reg='.$reg;
             
              }else{
                $and=base64_encode($and);
                $filtera=base64_encode($filtera);
              $urla='http://localhost/hontza/rssHontza/filter.php?url='.$url.'&filter='.$filtera.'&and='.$and.'&reg='.$reg;
              ...
              if(isset($not)){
                $and=base64_encode($and);
                $not=base64_encode($not);
                $urla='http://localhost/hontza/rssHontza/filter.php?url='.$url.'&and='.$and.'&not='.$not.'&reg='.$reg;
              }else{
                $and=base64_encode($and);
              $urla='http://localhost/hontza/rssHontza/filter.php?url='.$url.'&and='.$and.'&reg='.$reg;
              }
          }
      }else{
          if(isset($filtera)){
            if(isset($not)){
                $not=base64_encode($not);
                $filtera=base64_encode($filtera);
            $urla='http://localhost/hontza/rssHontza/filter.php?url='.$url.'&filter='.$filtera.'&not='.$not.'&reg='.$reg;
            }else{
              $filtera=base64_encode($filtera);
            $urla='http://localhost/hontza/rssHontza/filter.php?url='.$url.'&filter='.$filtera.'&reg='.$reg;
            }
        }else{
            if(isset($not)){
            $not=base64_encode($not);
            $urla='http://localhost/hontza/rssHontza/filter.php?url='.$url.'&not='.$not.'&reg='.$reg;
            }else{
            $urla='http://localhost/hontza/rssHontza/filter.php?url='.$url.'&reg='.$reg;
            }
        }
      }

   if (strlen($urla)>4500){
      $portzentaia=(strlen($urla)*100)/4500;
      $portzentaia=$portzentaia-100;
      drupal_set_message(t('The length of your filter has exceeded the limit by '.$portzentaia.'%. Please delete several RSS or filter terms.'));
      $nid=arg(1);
      $link=url($base_url.$my_lang.'/'.$grupo_link.'/node/'.$nid.'edit');
      drupal_goto($link);
     }
