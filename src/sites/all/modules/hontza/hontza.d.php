...
function hontza_gestion_grupos_propios_access(){
    global $user;
    if(user_access('root') || user_access('administrator') || hontza_is_creador_de_grupo()){
        return 1;
    }    
    //gemini-2014
...
function hontza_define_gestion_grupos_propios_sql($field,$sort,$is_admin_grupo_rol=0){
    global $user;
        ...
        if($user->uid==1 || $user->roles[ADMINISTRADOR]){//hemen aldatzen da zure grupo propioak azaltzeko edota besteenak ere bai.
...
function hontza_get_block_content_anadir_canal(){
	...
	$result[]=l(t('RSS'), 'crear/canal-yql',array('query'=>array('simple'=>1)));
  // yqla izorratuta dagoen bitartean
	...
	//intelsat-2015
        /*if(is_super_admin()){
            ...
            if(hontza_canal_json_is_activado()){          
                $result[]=l(t('Filter RSS'), 'canal_json/filtrorss');
                $result[]=l(t('CSV'), 'canal_json/crear_csv');
                $result[]=l(t('JSON'), 'canal_json/crear');               
                $result[]=l(t('Google Sheet'), 'canal_json/google_sheet');
            }
        //intelsat-2015
        //if(!hontza_canal_rss_is_visualizador_activado()){
            ...
            if(hound_enlazar_inc_is_activado()){
              ...
              $result[]=l(t('See Hound sources'), 'http://hound.hontza.es/hound2/public/sources/verFuentes/'.hound_enlazar_inc_get_key().'?subdominio='.get_base_url(), array('attributes'=>array('target'=>'_blank','title'=>t('Web'),'alt'=>t('Web')),'html'=>true)); 
             
              $result[]=l(t('Create Hound'), 'http://hound.hontza.es/hound2/public/channels/crearCanal/'.hound_enlazar_inc_get_key().'?subdominio='.get_base_url().'&grupo='.get_group(), array('fragment' => ' ', 'external' => TRUE));               
            }

            if(hontza_is_hound_actions()){                
                if(module_exists('hound')){

                  $result[]=l(t('Load Hound'), 'crear/canal-yql',array('query'=>array('simple'=>1,'is_hound'=>1)));
            ...
            //joseba-2018
             ...
             $result[]=l(t('List of channels'),'download_channels_txt',array('attributes'=>array('target'=>'_blank'))); 
            //intelsat-2016
...
function hontza_is_hound_2(){
      if(defined('_IS_HOUND_2') && _IS_HOUND_2==1){ 
        return 1;
    }
    return 0;
}
...
function hontza_item_no_link($node,$destination_in=''){
    //$label=t('Edit');
    $label='';
    $options=array('attributes'=>array('title'=>t('No link associated')));
    if(!empty($destination_in)){
        $options['query']=$destination_in;
    }
    return l($label,"node/".$node->nid.'/edit',$options);   
