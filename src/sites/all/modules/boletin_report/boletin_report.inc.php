            ...
            foreach($enlaces as $i=>$row){
                ...
                if(isset($my_node->nid) && !empty($my_node->nid)){
                    ...
                    if(in_array($node->type,array('item','noticia','wiki','idea','debate'))){
                        ...
                        //$kondizioa_notizi_hutsa = crear_crm_exportar_is_node_link_web($my_node);
                        //link-ik baduen edo ez ikusteko
                        if (!hontza_crm_is_show_link_type_action($my_node)){
                            $html[]='<td>';//nik gehituta 
                            //link eza
                            //images/icons/link_type_web.png
                            $html[]='<img src= "/sites/all/themes/buho/images/icons/no_link_associated.png" alt="No link associated to user news" title="No link associated to user news">';//nik gehituta 
                            $html[]='</td>';//nik gehituta 
                        }
                        elseif(crear_crm_exportar_is_node_link_web($my_node)){
                            //joseba2018
                        $html[]='<td>';//nik gehituta 
                        //irekita
                        //images/icons/link_type_web.png
                        $html[]='<img src= "/sites/all/themes/buho/images/icons/link_type_web.png" alt="Open link" title= "Open link">';//nik gehituta 
                        $html[]='</td>';//nik gehituta 
                        }else{
                        $html[]='<td>';//nik gehituta 
                        //itxita
                        //images/icons/link_type_platform.png
                        $html[]='<img src= "/sites/all/themes/buho/images/icons/link_type_platform.png" alt="Closed link" title="Closed link">';//nik gehituta 
                        $html[]='</td>';//nik gehituta  
                        }
                    }  
                    $html[]='<td>';    
