...
function alerta_variables_access(){
    global $user;
    if($user->uid==1 || $user->roles[ADMINISTRADOR]){
        return 1;
    }
    /*
    //gemini-2014
...
function alerta_get_bulletins_block_links(){
    ...
    if(isset($my_grupo->nid) && !empty($my_grupo->nid)){        
        ...
        $url2='boletin_report_automatized/add';
        if(boletin_report_is_permiso_editar($my_grupo->nid)){
            //intelsat-2015
            $icono=my_get_icono_action('add_left',t('New Series')).'&nbsp;';
            $create_new_url.=$icono.l(t('New Series'),$url); 
        } 

        if(boletin_report_is_permiso_editar($my_grupo->nid)){
            //intelsat-2015
           if(hontza_boletin_automatizado_is_activado()) {
            $icono=my_get_icono_action('add_left',t('New Automatic Series')).'&nbsp;';
            $create_new_url2.=$icono.l(t('New Automatic Series'),$url2); 
            }
        } 
